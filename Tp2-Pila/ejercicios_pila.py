...
#Ej necesario para el 14

def invertir_Pila_Sin_Perder_Elementos(pila_a_invertir):
    pila_Inversa = Pila()
    aux_Pila = Pila()

    #da como resultado la pila invertida
    ...
    #devuelvo elementos que desapilé a la pila original
...
#Ejercicio14 - Ingresar elem. de una pila

def insertar_Num_Menor_al_Mayor_En_Pila(obj_Pila_A_Insertar_Numero, numero):

    obj_Numeros_Mayores_Pila = Pila()

    #Si se ingresa mas de 1 numero entra aca
    if (not obj_Pila_A_Insertar_Numero.pila_vacia()):
        ...
        if (numero >= obj_Pila_A_Insertar_Numero.elemento_cima()):
            ...
            # Evaluo todos los menores al num incluyendo = y los apilo
            # En la pila auxiliar obj_Numeros_Mayores_Pila
            # Cuando se rompa el bucle, se q el numero es mayor al elemento de la cima o que no encontro ninguno
        ...
        #Si no hay nada, inserto el primer numero (para cuando se hace el primer bucle)
        obj_Pila_A_Insertar_Numero.apilar(numero)
...
def ejercicio_14():
    ...
    while(opcion == 'y'):
        ...
        insertar_Num_Menor_al_Mayor_En_Pila(obj_Pila, numero)
